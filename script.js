document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, errorType) {
      const label = field.closest('label, .form-group, .field-wrapper')
        ? (field.closest('label, .form-group, .field-wrapper').querySelector('label') || {}).textContent || field.name || field.placeholder
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('minlength') && field.value.trim() !== '') {
        ...
        if (!validators.minlength(field.value, minLen)) {
  ...
  const OBSERVE_SELECTORS = '.fade-in, .animate-on-scroll, .scroll-animate, [data-animate]';

  function applyScrollAnimations() {
    const elements = document.querySelectorAll(OBSERVE_SELECTORS);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מוטי וינקלר משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '1fbcf257-ec1a-4a1a-99bd-519b3709edda',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '1fbcf257-ec1a-4a1a-99bd-519b3709edda';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '1fbcf257-ec1a-4a1a-99bd-519b3709edda';
